...
print("Demo 17: Entrenar y Guardar un Modelo CNN de Clasificacion de Firmas Manuscritas")
...
print("2. Cargando el DataSet de Firmas")
ruta = "C:/Data/Python/2026_01_DAIAP/DataSets/Firmas/"
...
print("4. Creando el DataLoader de Firmas")
loaderTrain = DataLoader(dstTrain, batch_size=hpTamanoLote)

firmas = next(iter(loaderTrain))
firma = firmas[0]
print(f"Shape firma: {firma.shape}")

print("5. Crear un Modelo de Clasificacion")
modelo = CNN_3C3P(sizeImage, 1, 6).to(device)
...
for i in range(hpEpocas):
    if not encontro:
        for j,(X,y) in enumerate(loaderTrain):
            ...
            if(valor==0):
                encontro=True
                torch.save(modelo.state_dict(), "CNN_2C1P_Firmas_" + str(valor) + ".pt")
                break
