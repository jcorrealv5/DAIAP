...
class Ventana(QDialog):
    def __init__(self):
        ...
        self.modelo = CNN_3C3P(142, 1, 6).to(self.device)
        with open('CNN_2C1P_Firmas_0.0.pt', 'rb') as file: 
            self.modelo.load_state_dict(torch.load(file, map_location=self.device, weights_only=True))  
        rutaFirmas = "C:/Data/Python/2026_01_DAIAP/DataSets/Firmas/"
        self.personas = os.listdir(rutaFirmas)
        print(f"Personas: {self.personas}")
    ...
    def clasificarCaracteres(self):
        ...
        imgResize = cv2.resize(imgNdArray, (142, 142))
        ...
        with torch.no_grad():
            ...
            probabilidad = torch.max(y_pred).item()
            persona = self.personas[indice]
            self.lblPrediccion.setText(f"Persona: {persona} - Prob: {probabilidad:2f}")
    ...
    def mouseMoveEvent(self, e):
        ...
        grafico.setPen(QPen(Qt.white, 10, Qt.SolidLine))
