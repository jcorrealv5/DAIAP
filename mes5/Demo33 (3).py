...
with torch.no_grad():
    for j,(X,y_sexo,y_raza) in enumerate(loaderTrain):
        totalPersonas += y_sexo.numel()
        totalMujeres += y_sexo.sum()
        print(f"Total de Mujeres {j+1}: {totalMujeres}")   
totalHombres = totalPersonas - totalMujeres
...
for i in range(hpEpocas):
    ...
    if not encontro:
        for j,(X,y_sexo,y_raza) in enumerate(loaderTrain):
            X_train = X.to(device)
            y_sexo = y_sexo.to(device).reshape(hpTamanoLote,1).float()
            y_pred_sexo, y_pred_raza = modelo(X_train)
            lossSexo = criterioBin(y_pred_sexo, y_sexo)
...
with torch.no_grad():
    for j,(X,y_sexo,y_raza) in enumerate(loaderTest):
        ...
        predicSexo = (y_pred_sexo > 0.5).squeeze().int()
        _, predicRaza = y_pred_raza.max(1)
        num_correct += ((predicSexo == y_sexo) & (predicRaza == y_raza)).sum().item()
