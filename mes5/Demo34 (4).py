...
with open('CNN_6C3P3FC2S_SexoRaza_0.0428.pt', 'rb') as file:
    modelo.load_state_dict(torch.load(file, map_location=device, weights_only=True))
modelo.eval()

archivo = "Oficina2.mp4"
cap = cv2.VideoCapture(archivo)
if(cap.isOpened()):
    while True:
        rpta, imagen = cap.read()
        if(rpta):
            caras = clasificador.detectMultiScale(imagen, scaleFactor=1.05, minNeighbors=5, minSize=(100,100),flags=cv2.CASCADE_SCALE_IMAGE)
            if(len(caras)>0):
                for(x,y,w,h) in caras:
                    ...
                    predicSexo = (y_pred_sexo > 0.5).float().squeeze().int()
                    _, predicRaza = y_pred_raza.max(1)
                    sexo = ("Mujer" if predicSexo==1 else "Hombre")
            ...
            imagen = cv2.resize(imagen, (1000, 800))
